...
try{
const videoSale= await VideoSale.find({user:req.body.user})
.populate("video")

if(videoSale.length <=0)return res.status(404).json({error:true,errMessage:"You have not bought any book at the moment"})
res.status(200).json({error:false,message:videoSale})
}catch(error){

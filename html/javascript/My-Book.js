...
document.addEventListener("DOMContentLoaded",async()=>{
   ...
   const user=getCookie("identity")
        const response=await fetch("/api/book/sale/list",{
            method:"POST",
      ...
      console.log(result)
        ...
        result.message.forEach(item => {
           ...
           image.src=item.book.image_link;
           pricediv.className="price";
           pricediv.innerHTML=`₦${item.book.price.toString().replace(/\B(?=(\d{3})+(?!\d))/g,",")}`
